    ...
    if ($ramo->Lun == "x") {
        for ($j = 1; $j <= 19; $j++) {
            if ((strtotime($ramo->HorInic) <= strtotime($inicioB[$j])) && (strtotime($ramo->Final) >= strtotime($finalB[$j]))) {
                ...
                $DocenteM[$j][1] = $ramo->NombreDocente;
        ...
        for ($j = 1; $j <= 19; $j++) {
            if ((strtotime($ramo->HorInic) <= strtotime($inicioB[$j])) && (strtotime($ramo->Final) >= strtotime($finalB[$j]))) {
                ...
                $DocenteM[$j][2] = $ramo->NombreDocente;
        ...
        for ($j = 1; $j <= 19; $j++) {
            if ((strtotime($ramo->HorInic) <= strtotime($inicioB[$j])) && (strtotime($ramo->Final) >= strtotime($finalB[$j]))) {
                ...
                $DocenteM[$j][3] = $ramo->NombreDocente;
        ...
        for ($j = 1; $j <= 19; $j++) {
            if ((strtotime($ramo->HorInic) <= strtotime($inicioB[$j])) && (strtotime($ramo->Final) >= strtotime($finalB[$j]))) {
                ...
                $DocenteM[$j][4] = $ramo->NombreDocente;
        ...
        for ($j = 1; $j <= 19; $j++) {
            if ((strtotime($ramo->HorInic) <= strtotime($inicioB[$j])) && (strtotime($ramo->Final) >= strtotime($finalB[$j]))) {
                ...
                $DocenteM[$j][5] = $ramo->NombreDocente;
        ...
        for ($j = 1; $j <= 19; $j++) {
            if ((strtotime($ramo->HorInic) <= strtotime($inicioB[$j])) && (strtotime($ramo->Final) >= strtotime($finalB[$j]))) {
                ...
                $DocenteM[$j][6] = $ramo->NombreDocente;
        ...
        ?>
                <td id="<?php echo "bloque" . $ie . "dia" . $im; ?>" class="fondo_<?php if(isset($asignaturaM[$ie][$im])){echo $asignaturaM[$ie][$im];} ?>">
                    <table class="table table-sm table-borderless">
                        ...
                        <br><?php if (isset($DesAsignatura[$ie][$im])){echo $DesAsignatura[$ie][$im];} ?>
                        </td>
                            <td class="fondo_<?php if(isset($asignaturaM[$ie][$im])){echo $asignaturaM[$ie][$im];} ?>"><small><?php if(isset($AulaM[$ie][$im])){echo substr($AulaM[$ie][$im], 3);} ?></small><br /><small><?php if (isset($CapacidadM[$ie][$im])){echo $CapacidadM[$ie][$im];} ?></small></td>
                        </tr>
                        <tr>
...
<script>
    $(document).ready(function () {
        $(".editarLink").click(function (event) {
            ...
            $('.modal').load('<?php echo base_url(); ?>index.php/programacion/editarasignatura/' + strEditar);
            $('.modal').modal('show').one('click', '#delete', function (e) {
                var strEditar2 = $("#delete").val();
                var strEditar3 = $("#seccionx").val();
                $.post("<?php echo base_url(); ?>index.php/programacion/borrarasignatura/", {IdAsig: strEditar2, time: "2pm"}).done(function (e) {
                    //alert(e);
                    $('.modal').modal('hide');                    
                    $("#verHorario").load("<?php echo base_url(); ?>index.php/programacion/horario/" + strEditar3);
                });
            });
//alert(strEditar);
        });
